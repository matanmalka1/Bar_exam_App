...
def upgrade() -> None:
    op.create_table(
        ...
        sa.CheckConstraint(
            "length(trim(display_name)) > 0", name="ck_users_display_name_not_empty"
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "answered_count", sa.Integer(), nullable=False, server_default=sa.text("0")
        ),
        ...
        sa.CheckConstraint(
            "mode IN ('exam', 'practice', 'mistakes')", name="ck_sessions_mode"
        ),
        sa.CheckConstraint(
            "status IN ('active', 'completed', 'abandoned')", name="ck_sessions_status"
        ),
        sa.CheckConstraint(
            "part IS NULL OR part IN ('B', 'C')", name="ck_sessions_part"
        ),
        ...
        sa.CheckConstraint(
            "answered_count <= total_questions", name="ck_sessions_answered_le_total"
        ),
        sa.PrimaryKeyConstraint("id"),
    )
    op.create_index(
    ...
    op.create_index(
        "ix_psq_session_id", "practice_session_questions", ["session_id"], unique=False
    )
    ...
    op.create_table(
        ...
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint(
            "session_id", "question_id", name="uq_user_answers_session_question"
        ),
    )
    ...
    op.create_index(
        "ix_bookmarks_user_id", "bookmarked_questions", ["user_id"], unique=False
    )
